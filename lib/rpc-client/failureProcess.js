...
function failover(code, tracer, serverId, msg, opts, cb) {
    ...
    if (!servers.length) {
        logger.error('[dreamix-rpc] rpc failed with all this type of servers, with serverType: %s', serverType);
 ...
 * @api private
 */
function failsafe(code, tracer, serverId, msg, opts, cb) {   // eslint-disable-line
    const self = this;
 ...
 * @api private
 */
function failback(code, tracer, serverId, msg, opts, cb) { // eslint-disable-line
...
const exp = (code, tracer, serverId, msg, opts) => {
    const cb = tracer.cb;
